...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("BRC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("branch:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BRC unit input in lnReqAmount, we generate the URI with BRC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BRC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
